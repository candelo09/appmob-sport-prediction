import { getByParticipantGroupStanding } from "@/src/services/participant-group-standing-service";
import { useQuery } from "@tanstack/react-query";

export const useParticipantGroupstandingByGroup = (
  participantId: number,
  groupId: number,
) => {
  const allGroupStandingByGroup = useQuery({
    queryKey: ["groupId", groupId],
    queryFn: () => getByParticipantGroupStanding(participantId, groupId),
